...
int gevtID, nInitParticles;
int initpdg[100];
float initx[100], inity[100], initz[100], initpx[100], initpy[100], initpz[100];
double inittime;
...
double GetNdf(TTree* ttree, int isWeight);

struct fitRes myroofit(TTree* ttree, TH1F* h2, int evttag, int fittag, int isWeight, double ndf00);
...
void anafs_atm_roofitunbinnedML_2tau(int fileNo)
{
  ...
  if (fileNo < 10) {
    ...
    ID = "000";
    ID += fileNo;
  } else if (fileNo < 10000) {
    ID = "00";
    ID += fileNo;
  }

  TString datapath = "/dybfs/users/chengjie/data/PSDTools/J21/AtmNC/Model-G";
  TString datapath1 = "/dybfs/users/chengjie/data/PSDTools/J21/AtmNC/Model-G";
  TString fileName1 = datapath1 + "/DS/user_" + ID + ".root";
  ...
  geninfo->SetBranchAddress("nInitParticles", &nInitParticles);
  ...
  TString outName1 = datapath + "/Ana/atmhist_" + ID + ".root";  //--J21
  TString outName2 = datapath + "/Ana/atmfit_" + ID + ".root";   //--J21
  ...
  t_psd->Branch("iso_Z", &iso_Z, "iso_Z/I");
  t_psd->Branch("iso_N", &iso_N, "iso_N/I");
  ...
  t_psd->Branch("npars", &npars, "npars/I");
  t_psd->Branch("m_initpdg", m_initpdg, "m_initpdg[npars]/I");
  t_psd->Branch("m_initpx", m_initpx, "m_initpx[npars]/D");
  t_psd->Branch("m_initpy", m_initpy, "m_initpy[npars]/D");
  t_psd->Branch("m_initpz", m_initpz, "m_initpz[npars]/D");
  ...
  for (int i = 0; i < m_elecID.size(); i++) {
    ...
    if (m_elec_temID.size() == 2) {
      m_elecIBDlikeID.push_back(m_elec_temID[0]);
    }
    if (m_elec_temID.size() > 2) {
      double dt = m_elec_temID[2].trigTime_evt - m_elec_temID[0].trigTime_evt;
      if (dt > 1.) {
        m_elecIBDlikeID.push_back(m_elec_temID[0]);
      }
    }
  ...
  for (int ii = 0; ii < m_elecIBDlikeID.size(); ii++) {
    ...
    if (m_timeBuffer.size() >= 2) {
      ...
      if (pEvt.qe_evt > 10. && pEvt.pdg_evt != 2112 && nm == 1) {
        ...
        npars = 0;
        npars = nInitParticles;

        for (int nn = 0; nn < nInitParticles; nn++) {
          m_initpdg[nn] = initpdg[nn];
          m_initpx[nn] = initpx[nn];
          m_initpy[nn] = initpy[nn];
          m_initpz[nn] = initpz[nn];
        }
        nPE_o = 0.;
        iso_Z = 0;
        iso_N = 0;
        ...
        for (int nn = 0; nn < nInitParticles; nn++) {
          if (initpdg[nn] > 1000030000) {
            iso_Z = int((initpdg[nn] - 1000000000) / 10000);
            iso_N = int(initpdg[nn] - 1000000000 - iso_Z * 10000);
            iso_N = iso_N / 10;
          }
        }
        double nhits = 0.;
        ...
        if (dr3 < dr3cut && Eqen_o > 10.) {
          ...
          double ndf1 = GetNdf(treeAll_tem, 0);
          struct fitRes res1 = myroofit(treeAll_tem, timetest5_fs1, evtID_n, 0, 0, ndf1);
          tau1_fs1 = res1.tau1;
          tau2_fs1 = res1.tau2;
          //tau3_fs1 = res1.tau3;
          ...
          double ndf2 = GetNdf(treeAll_tem, 1);
          struct fitRes res2 = myroofit(treeAll_tem, timetest5_fs2, evtID_n, 1, 1, ndf2);
          tau1_fs2 = res2.tau1;
          tau2_fs2 = res2.tau2;
          //tau3_fs2 = res2.tau3;
...
void PMTPosition()
{
  TString filename = "/junofs/users/chengjie/workdir/PSDTools/Ana-new0817/input/PMTPos_Acrylic_with_chimney.csv";
  ifstream sfile;
